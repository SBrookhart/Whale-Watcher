...
const FALLBACK_BLOCKS = 5000; // wider when falling back
...
/** ---------- Alchemy with pagination + fromBlock ---------- */
...
/** ---------- Fallback: RPC getLogs per token (wide window) ---------- */
...
export default async function handler(req) {
  ...
  try {
    ...
    try { items = await fallbackErc20(minUsd, usdUSDC, usdUSDT); } catch { items = []; }
  }
